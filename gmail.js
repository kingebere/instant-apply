...
function promisifiedGenerateRandomString(length) {
...
class gmailMainScript {
	...
	async fillnameEmailCompanyLinkedIn(
		...
		this.mailID = await promisifiedGenerateRandomString(10);
		const {
			data: {
				session: { user },
			},
  ...
  `
		...
		firstContent.className = "list";
		firstContent.setAttribute(
			"style",
			`
  align-items: center;
  flex-direction: row;
  ...
  `
		...
		secondContent.className = "list";
		secondContent.setAttribute(
			"style",
			`
  align-items: center;
  flex-direction: row;
  ...
  `
		...
		thirdContent.className = "list";

		thirdContent.setAttribute(
			"style",
			`
  align-items: center;
  flex-direction: row;
...
`
		...
		Array.from(document.querySelectorAll(".list")).forEach((node) => {
			node.addEventListener("mouseover", () => {
				node.style.backgroundColor = "#006aff";
